...
export default function ResetPasswordPage() {
  ...
  // true while Supabase is exchanging the token from the URL hash
  const [sessionReady, setSessionReady] = useState(false);

  // Supabase embeds the recovery token in the URL hash on redirect.
  // Calling getSession() triggers the SDK to parse the hash and establish
  // a temporary recovery session, after which updateUser() is allowed.
  useEffect(() => {
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden">
          <div className="px-7 pt-7 pb-5 border-b border-slate-800">
            <h2 className="text-lg font-bold text-white leading-tight">Set a new password</h2>
