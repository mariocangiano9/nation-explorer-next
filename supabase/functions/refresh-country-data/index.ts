...
serve(async (req: Request) => {
  ...
  if (language === "en") {
    // Step 1: Generate full profile in English
    ...
    try {
      claudeResponse = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 16000,
          system: `You are an expert geopolitical and economic analyst. Reply ONLY with a valid JSON object, without additional text, comments, or markdown formatting. All text values must be in English.`,
          messages: [{ role: "user", content: buildPrompt(countryName, "en") }],
        }),
      });
    } catch (err) {
      return new Response(
    ...
    // Step 2: For non-English languages, fetch English data and translate
    ...
    if (fetchError || !englishRow) {
      return new Response(
        JSON.stringify({
          error: "English data not found. Please generate English profile first.",
    ...
    const englishData = englishRow.data;
    ...
    try {
      claudeResponse = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 16000,
          system: `You are a translator. Translate ONLY the text fields of this JSON to ${langName}. Keep ALL numeric values, codes, ISO codes, flag emojis, dates, percentages, country names in arrays, and organization acronyms exactly as they are. Return ONLY the complete translated JSON object.`,
          messages: [{ role: "user", content: `Translate this JSON to ${langName}: ${JSON.stringify(englishData)}` }],
        }),
      });
    } catch (err) {
      return new Response(
    ...
    if (!parsed) {
      return new Response(
        JSON.stringify({ error: "Failed to parse translated response as JSON" }),
        { status: 502, headers: { "Content-Type": "application/json" } },
      );
    }
